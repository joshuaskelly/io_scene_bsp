...
from typing import NamedTuple, Tuple, List, Protocol
...
class KdRegionTree(object):
    ...
    def insert(self, element: Region):
...
PackResult = NamedTuple('PackResult', atlas_size=Tuple[int, int], offsets=List[Tuple[int, int]])


def pack(regions: List[Region], size: Tuple[int, int] = None) -> PackResult:
    if not size:
        area = sum([r.size[0] * r.size[1] for r in regions])
        side = 1 << (math.ceil(math.sqrt(area)) - 1).bit_length()
        size = side, side
